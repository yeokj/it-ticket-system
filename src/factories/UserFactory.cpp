...
std::shared_ptr<User> UserFactory::createUser(const std::string& type, const std::vector<std::string>& tokens) {
    try {
        if (type == "Client") {
            ...
            std::string company = tokens[4];

            return std::make_shared<Client>(id, name, email, password, company);
        } 
            ...
            std::string email = tokens[2];
            std::string password = tokens[3];
            int emplID = std::stoi(tokens[4]);
            std::string department = tokens[5];

            return std::make_shared<Technician>(id, name, email, password, emplID, department);
        } 
        else {
